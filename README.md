...
**Freigeben:** System → Linkmap → Einstellungen → Datensatz-Quellen. Pro Tabelle: Label-Template (`{title} ({date})`), Suchspalten, Spalten der Listenansicht, Sortierung, Filter, Sprachspalte, erlaubte URL-Schemata, URL-Template. Leer gelassene Felder nutzen die YForm-Felddefinitionen. Die Listenzellen zeigen Anzeigewerte wie die YForm-Datenliste: Relationen (be_manager_relation) als Name des verknüpften Datensatzes, Auswahlfelder als Label, Datum und Datum/Zeit in der Kurzform der Backend-Sprache (`rex_formatter::intlDate`), sortiert wird trotzdem nach der Rohspalte. Die Sprachwahl im Picker erscheint für eine Tabelle nur, wenn eine Sprachspalte gesetzt ist. Die Link-Spalte der Liste zeigt die Art der URL (Profil- bzw. Schema-Label), die konkreten URLs stehen im Tooltip. Rechte über `yform_manager_table_view` / `_edit`. Im Relation-Modus muss die Tabelle nicht freigegeben sein, das Feld legt sie fest.
